...
void setEndGameScoreboard(PatchGameConfig_t * gameConfig)
{
	...
	int i;
	int topTeamIdx = State.Teams[1].TeamId == State.WinningTeam;
	if (State.WinningTeam < 0)
		topTeamIdx = State.Teams[1].TeamId < State.Teams[0].TeamId;
	...
	for (i = 0; i < GAME_MAX_PLAYERS; ++i)
	{
		...
		struct PayloadPlayer* pState = &State.PlayerStates[pid];

...
//--------------------------------------------------------------------------
int controllerControlTeleportPlayer(Moby* moby, struct ControllerTarget* target)
{
  if (!target->TeleportPlayer.PlayerMask) return 0;

  Player** players = playerGetAll();
  int i;
  int count = 0;
  int acceptsHost = (target->TeleportPlayer.PlayerMask & CONTROLLER_PLAYER_MASK_HOST) && controllerAmIOwner(moby);
  for (i = 0; i < GAME_MAX_PLAYERS; ++i) {
    Player* player = players[i];
    if (!player || !player->PlayerMoby) continue;

    int bit = 1 << i;
    if ((target->TeleportPlayer.PlayerMask & bit) != 0 || (acceptsHost && player->IsLocal)) {
      
      SpawnPoint* sp = spawnPointGet(target->TeleportPlayer.TeleportToCuboidIdx);
...
int controllerControlCompleteMission(Moby* moby, struct ControllerTarget* target)
{
  ...
  return 1;
#elif OBSTACLE
  if (!MapConfig.SetLocalPlayerReachedEnd) return 0;

  MapConfig.SetLocalPlayerReachedEnd();
  DLOG(moby, "mission end\n");
  return 1;
#else
  return 0;
...
int controllerIterate(Moby* moby)
{
  ...
  for (i = 0; i < CONTROLLER_MAX_TARGETS; ++i) {
    ...
    switch (pvars->Targets[i].TargetUpdateType) {
      ...
      case CONTROLLER_TARGET_UPDATE_TYPE_PLAYER_TELEPORT: changed += controllerControlTeleportPlayer(moby, &pvars->Targets[i]); break;
    }
  }

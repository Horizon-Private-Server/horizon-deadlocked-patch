...
int leviathanCreate(int spawnParamsIdx, VECTOR position, float yaw, int spawnFromUID, int spawnFlags, struct MobConfig *config)
{
	...
	guberMobyCreateSpawned(LEVIATHAN_MOBY_OCLASS, sizeof(struct MobPVar) + sizeof(LeviathanMobVars_t), &guberEvent, NULL);
	if (guberEvent)
	{
...
int leviathanCanNonOwnerTransitionToAction(Moby* moby, int action)
{
  // only let owner choose actions for boss
...
int leviathanShouldForceStateUpdateOnAction(Moby* moby, int action)
{
  // only let owner choose actions for boss
  // since for this boss it is critical it syncs well
  if (leviathanIsBoss(moby)) return 1;

 ...
 * 
 * DESCRIPTION :
 * 		Custom map logic for Survival Valix.
 * 		
...
#define BOSS_ARENA_JUMPPAD_MOBY_UID     (62)
...
void mapOnEndBossFight(void)
{
  ...
  for (i = 0; i < GAME_MAX_LOCALS; ++i) {
    ...
    if (!player || !player->PlayerMoby || !playerIsConnected(player)) break;
  ...
  Moby* moby = mobyListGetStart();
    ...
    if (count >= BOSS_ARENA_WEP_PICKUP_COUNT) break;
		++moby;
	}
...
void mapOnBeginBossFight(void)
{
  ...
  memset(BossHackerOrbsStates, 0, sizeof(BossHackerOrbsStates));
    ...
    if (count >= BOSS_ARENA_HACKERORB_COUNT) break;
		++moby;
	}
...
void mapOnBossFight(void)
{
  ...
  if (IsInBossFight == 2) {
    ...
    if (spawnMinions && mobsLeft < BOSS_ARENA_MAX_MINIONS) {
      ...
      if (MapConfig.State->RoundSpawnTicker < 0) MapConfig.State->RoundSpawnTicker = 1;
...
//--------------------------------------------------------------------------
void mapCheckForBossFightStateChange(void)
{
...
int mapCanSpawnMobs(void)
{
  ...
  if (!IsInBossFight) return 1;

  // guarantee all spawn points in arena are used
  bakedConfig.SpawnDistanceFactor = 0;
...
void mapReturnPlayersToMap(void)
{
  ...
  for (i = 0; i < GAME_MAX_LOCALS; ++i) {
    ...
    while (areaIdx <= OOB_AREA_INDEX_END) {
      if (!areaGetArea(areaIdx, &area)) break;
      if (area.CuboidCount <= 1) break;
      if (!area.Cuboids) break;
...
//--------------------------------------------------------------------------
void hopBigAl(VECTOR position, float yaw)
{
...
//--------------------------------------------------------------------------
int onHopBigAlRemote(void* connection, void* data)
{
  VECTOR p;
  float yaw;
  memcpy(p, data, 12);
...
int createMob(int spawnParamsIdx, VECTOR position, float yaw, int spawnFromUID, int spawnFlags, struct MobConfig *config)
{
  switch (spawnParamsIdx)
  {
    ...
    case MOB_SPAWN_PARAM_LEVIATHAN_COMMON:
...
struct GuberMoby* mapGetGuber(Moby* moby)
{
  ...
  switch (moby->OClass)
  {
    case SOULCOLLECTOR_OCLASS: return soulcollectorGetGuber(moby);
...
int mapHandleGuberEvent(Moby* moby, GuberEvent* event)
{
  ...
  switch (moby->OClass)
  {
    ...
    case SOULCOLLECTOR_OCLASS: return soulcollectorHandleEvent(moby, event);
...
void initialize(void)
{
  ...
  netInstallCustomMsgHandler(CUSTOM_MSG_TELEPORT_BIG_AL, &onHopBigAlRemote);
...
int main (void)
{
  ...
  if (MapConfig.State) {
    ...
    if (MapConfig.State->RoundEndTime == -1)

...
MenuElem_t menuElementsGameSettings[] = {
  ...
  { "Instant Death", toggleActionHandler, menuStateAlwaysEnabledHandler, &gameConfig.grInstantDeath, "Removes death animation delay." },
  ...
  { "Spawn Immunity", toggleInvertedActionHandler, menuStateAlwaysEnabledHandler, &gameConfig.grNoSpawnImmunity, "Toggles spawn immunity." },
...
void drawMapsListVerticalMenuElementInfo(TabElem_t* tab, MenuElem_t* element, MenuElem_VerticalListData_t * listData, RECT* rect)
{
  ...
  if (mapOverrideSelectedMapAuthor[0]) {
    ...
    gfxScreenSpaceBox(r.BottomLeft[0], r.BottomLeft[1] - LINE_HEIGHT, r.BottomRight[0] - r.BottomLeft[0], LINE_HEIGHT, 0x80181818);
    gfxScreenSpaceText(x, y, 1, 1, color, authorBuf, -1, TEXT_ALIGN_BOTTOMLEFT);
  }
}
...
{
  // unload thumbnail
  if (mapOverrideSelectedMapThumbnail) {

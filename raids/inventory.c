...
//--------------------------------------------------------------------------
enum InventoryItemAction inventoryDrawFooter(InventoryDrawState_t* drawState)
{
  ...
  if (padGetButtonDown(0, PAD_CROSS) > 0) {                             // EQUIP
    ...
    return INVENTORY_ITEM_ACTION_FAV;
  }

  return INVENTORY_ITEM_ACTION_NONE;
}
...
void inventoryDraw(void)
{
  ...
  // draw footer
  int itemAction = inventoryDrawFooter(&inventoryDrawState);

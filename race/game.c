...
struct RaceConfig Config __attribute__((section(".config"))) = {
	...
	.CheckPoints = {
		...
		{ 522.77, 272.76, 143.84, 92.81549 },
		{ 427.9, 456.36, 116.73, 40.12009 },
...
void processPlayer(int pIndex)
{
	...
	if (vector_sqrmag(t1) < (CHECKPOINT_RADIUS * CHECKPOINT_RADIUS) && vector_innerproduct_unscaled(t1, t2) < (CHECKPOINT_HIT_DEPTH * CHECKPOINT_HIT_DEPTH)) {

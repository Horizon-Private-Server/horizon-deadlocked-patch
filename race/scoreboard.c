...
void initializeScoreboard(int targetPoints)
{
	*(u32*)0x002FA084 = targetPoints;
...
void setEndGameScoreboard(PatchGameConfig_t * gameConfig)
{
	...
	int i;
	int topTeamIdx = State.WinningTeam;
	if (topTeamIdx < 0)
		topTeamIdx = 0;
	...
	for (i = 0; i < GAME_MAX_PLAYERS; ++i)
	{
		...
		struct RacePlayer* pState = &State.PlayerStates[pid];

		// set points
		sprintf((char*)(uiElements[22 + (i*4) + 2] + 0x60), "%d", (int)pState->Stats.LapNumber);
	}
}
